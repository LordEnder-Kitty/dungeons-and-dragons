...
public class MyBombBlock extends Block implements DetonatableBlock {
    public MyBombBlock(Settings settings) {
        super(settings);
    }
    ...
    private static void prime(World world, BlockPos pos, LivingEntity igniter) {
        if (!world.isClient && world.getBlockState(pos).getBlock() instanceof MyBombBlock) {
            MyBombEntity entity = ModEntities.MY_BOMB.create(world);
