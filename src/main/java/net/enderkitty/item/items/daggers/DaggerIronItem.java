...
public class DaggerIronItem extends Item implements Vanishable {
    ...
    public DaggerIronItem(Item.Settings settings) {
        super(settings);
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            if (j == 0) {
                DaggerEntityIron daggerEntityIron = new DaggerEntityIron(world, (LivingEntity)playerEntity, stack);
                daggerEntityIron.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0f, 2.5f + (float)j * 0.5f, 1.0f);
                if (playerEntity.getAbilities().creativeMode) {
                    daggerEntityIron.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                }
                world.spawnEntity(daggerEntityIron);
                world.playSoundFromEntity(null, daggerEntityIron, SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0f, 1.0f);
                if (!playerEntity.getAbilities().creativeMode) {
