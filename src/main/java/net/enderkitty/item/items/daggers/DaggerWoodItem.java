...
import net.enderkitty.entity.entities.DaggerEntityWood;
...
public class DaggerWoodItem extends Item implements Vanishable {
    ...
    public DaggerWoodItem(Settings settings) {
        super(settings);
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            if (j == 0) {
                DaggerEntityWood daggerEntityWood = new DaggerEntityWood(world, (LivingEntity)playerEntity, stack);
                daggerEntityWood.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0f, 2.5f + (float)j * 0.5f, 1.0f);
                if (playerEntity.getAbilities().creativeMode) {
                    daggerEntityWood.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                }
                world.spawnEntity(daggerEntityWood);
                world.playSoundFromEntity(null, daggerEntityWood, SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0f, 1.0f);
                if (!playerEntity.getAbilities().creativeMode) {
