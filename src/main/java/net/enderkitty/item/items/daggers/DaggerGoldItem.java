...
import net.enderkitty.entity.entities.DaggerEntityGold;
...
public class DaggerGoldItem extends Item implements Vanishable {
    ...
    public DaggerGoldItem(Settings settings) {
        super(settings);
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            if (j == 0) {
                DaggerEntityGold daggerEntityGold = new DaggerEntityGold(world, (LivingEntity)playerEntity, stack);
                daggerEntityGold.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0f, 2.5f + (float)j * 0.5f, 1.0f);
                if (playerEntity.getAbilities().creativeMode) {
                    daggerEntityGold.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                }
                world.spawnEntity(daggerEntityGold);
                world.playSoundFromEntity(null, daggerEntityGold, SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0f, 1.0f);
                if (!playerEntity.getAbilities().creativeMode) {
