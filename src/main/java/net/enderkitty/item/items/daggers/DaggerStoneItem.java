package net.enderkitty.item.items.daggers;

import com.google.common.collect.ImmutableMultimap;
import com.google.common.collect.Multimap;
import net.enderkitty.entity.entities.DaggerEntityStone;
...
public class DaggerStoneItem extends Item implements Vanishable {
    ...
    public DaggerStoneItem(Settings settings) {
        super(settings);
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            if (j == 0) {
                DaggerEntityStone daggerEntityStone = new DaggerEntityStone(world, (LivingEntity)playerEntity, stack);
                daggerEntityStone.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0f, 2.5f + (float)j * 0.5f, 1.0f);
                if (playerEntity.getAbilities().creativeMode) {
                    daggerEntityStone.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                }
                world.spawnEntity(daggerEntityStone);
                world.playSoundFromEntity(null, daggerEntityStone, SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0f, 1.0f);
                if (!playerEntity.getAbilities().creativeMode) {
