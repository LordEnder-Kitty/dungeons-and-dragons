...
public class DaggerDiamondItem extends Item implements Vanishable {
    ...
    public DaggerDiamondItem(Settings settings) {
        super(settings);
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            if (j == 0) {
                DaggerEntityDiamond daggerEntityDiamond = new DaggerEntityDiamond(world, (LivingEntity)playerEntity, stack);
                daggerEntityDiamond.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0f, 2.5f + (float)j * 0.5f, 1.0f);
                if (playerEntity.getAbilities().creativeMode) {
                    daggerEntityDiamond.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                }
                world.spawnEntity(daggerEntityDiamond);
                world.playSoundFromEntity(null, daggerEntityDiamond, SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0f, 1.0f);
                if (!playerEntity.getAbilities().creativeMode) {
